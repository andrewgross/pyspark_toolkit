...
# I have gravely confused the representations of how b"0" vs b"\x00" vs bytes([0]) works
...
def test_prepare_short_key():
    ...
    df = df.withColumn("key", _prepare_key(F.col("value"), block_size=block_size))
    expected_result = (
        b"e179017a-62b0-4996-8a38-e91aa9f1e179017a-62b0-4996-8aaaaaaaaaa\x00\x00"
    )
...
def test_prepare_long_key():
    ...
    df = spark.createDataFrame(data, T.BinaryType())
    df = df.withColumn("key", _prepare_key(F.col("value"), block_size=block_size))
    expected_result = b"W)nY\x1a%7!\xc5\x9e\x93\xcaI\xbd\xca5@{X\xdcx\xa2\x7f\xf31E\x03\xd3>\x15;'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    assert expected_result == bytes(df.collect()[0]["key"])
